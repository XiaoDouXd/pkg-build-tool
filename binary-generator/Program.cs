...
internal static class Program
{
    ...
    private static string BinToCpp(byte[] bytes, int offset, int len, string name, string outputPath)
    {
        var isWhole = offset == 0 && len == bytes.Length;
        var srcName = isWhole ? $"SRC_{name}" : $"SRC{offset}_{name}";
...
namespace RC {{
    const std::array<uint8_t, {len + 1}> __{srcName} = {{ ");
        for (var i = offset; i < len; i++)
        {
            cpp.Append($"0x{bytes[i]:x2}, ");
        }
        cpp.Append("0x00, ");
        cpp.Append("};\n");
        cpp.Append(
...
namespace RC {{
    ...
    private static void FileToCpp(string path)
    {
        ...
        {
            srcList.Add(BinToCpp(bytes, offset, maxBlockSize, name, output));
            offset += maxBlockSize;
        }
        if (bytes.Length - offset > 0) srcList.Add(BinToCpp(bytes, offset, bytes.Length - offset, name, output));
        if (srcList.Count > 0) CreateMap(srcList, name, output);
    }
